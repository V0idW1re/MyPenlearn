#!/usr/bin/env bash
# engagement_snapshot.sh — gather everything I need to audit a Penlearn engagement
...
#
# Output: /tmp/penlearn-snapshot-<project>-<timestamp>.tar.gz
set -e
...
if [ -z "$PROJECT" ]; then
  ...
  ls -1 ~/penlearn/projects/ 2>/dev/null || echo "  (none yet)"
  exit 1
fi

WORKSPACE_DIR="$HOME/penlearn/projects/$PROJECT/workspace"
...
TS="$(date +%Y%m%d-%H%M%S)"
SNAP_ROOT="/tmp/penlearn-snapshot-$PROJECT-$TS"
mkdir -p "$SNAP_ROOT"
...
PENLEARN_DB="$HOME/.local/share/penlearn-local/penlearn.db"
if [ -f "$PENLEARN_DB" ]; then
  sqlite3 "$PENLEARN_DB" ".dump chat_messages" > "$SNAP_ROOT/chat_messages.sql" 2>/dev/null || true
  sqlite3 "$PENLEARN_DB" ".dump agent_sessions" > "$SNAP_ROOT/agent_sessions.sql" 2>/dev/null || true
  sqlite3 "$PENLEARN_DB" ".dump agent_messages" > "$SNAP_ROOT/agent_messages.sql" 2>/dev/null || true
  sqlite3 "$PENLEARN_DB" ".dump findings" > "$SNAP_ROOT/findings.sql" 2>/dev/null || true
  sqlite3 "$PENLEARN_DB" ".dump execution_results" > "$SNAP_ROOT/execution_results.sql" 2>/dev/null || true
...
[ -f /tmp/penlearn-local.log ] && cp /tmp/penlearn-local.log "$SNAP_ROOT/app.log"

# 4. Versions snapshot so I know exactly what build behaved this way
{
  echo "=== Penlearn version ==="
  dpkg -l penlearn-local 2>/dev/null | tail -1
  echo ""
  echo "=== Git commit ==="
  cd /home/kali/penlearn-local 2>/dev/null && git rev-parse HEAD 2>/dev/null
  echo ""
  echo "=== Claude version ==="
  ~/.local/bin/claude --version 2>/dev/null || echo "(claude CLI not on PATH)"
  echo ""
  echo "=== MCP tool count ==="
  /usr/lib/penlearn-local/mcp-server/.venv/bin/python -c "from penlearn_mcp.tools.register_all import get_tool_definitions; print(len(get_tool_definitions()))" 2>/dev/null
...
# 6. Tarball it
TAR="/tmp/penlearn-snapshot-$PROJECT-$TS.tar.gz"
